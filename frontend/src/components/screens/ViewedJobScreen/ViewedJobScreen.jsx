...
export default function viewedJobsScreen({ onNavigate }) {
  ...
  useEffect(() => {
    const sync = () => {
      const t = getToken()
      if (!t) {
        navigate('/login?returnUrl=/viewed-jobs&msg=session_expired')
        return
      }
      setToken(t)
    }
  ...
  const handleJobClick = (job) => {
    ...
    navigate(`/viewed-jobs/job/${job.jobID}`)
  }
  ...
  const renderJobCard = (job) => (
    <div key={job.jobID} style={{ cursor: 'pointer' }}
      ...
      <JobCard
        key={`${job.jobID}-${viewedJobIds.has(job.jobID)}`}
        token={token}
        job={{
          id: job.jobID,
          title: job.title,
          ...
          isviewed: viewedJobIds.has(job.jobID),
          isExpired: job.isExpired
        }}
        onSave={(jobID) => {
          if (!viewedJobIds.has(jobID)) trackBehavior(jobID, 'save')
        }}
  ...
  return (
    <div className="sj-page">
      <div className="sj-layout">
        <Sidebar activeItem="viewed-jobs" onNavigate={onNavigate} />
        ...
        <main className="sj-main">
          ...
          <div className="sj-header">
            <div className="sj-header-inner">
              <div>
                <h1 className="sj-title">Việc làm đã xem</h1>
              ...
              <div className="sj-header-stats">
                <div className="sj-hstat">
                  ...
                  <span className="sj-hstat-l">Đã xem</span>
                </div>
              </div>
            </div>
          </div>
